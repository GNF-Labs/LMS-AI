...
def get_single_item_recommendation(csv_file, user_id, item_id, model_name='jodie', gpu_id=-1, epoch=48, 
    ...
    # Tidak perlu set GPU environment, karena kita akan menggunakan CPU
    ...
    model = JODIE(args, num_features=len(feature_sequence[0]), num_users=num_users, num_items=num_items).cpu()
    ...
    set_embeddings_training_end(user_embeddings_dystat, item_embeddings_dystat, user_embeddings_timeseries, item_embeddings_timeseries, user_sequence_id, item_sequence_id, train_end_idx_training) 
    ...
    # GET EMBEDDINGS FOR SPECIFIED USER AND ITEM (di CPU)
    user_embedding_input = user_embeddings[torch.LongTensor([user_id])]
    user_embedding_static_input = user_embeddings_static[torch.LongTensor([user_id])]
    item_embedding_input = item_embeddings[torch.LongTensor([item_id])]
    item_embedding_static_input = item_embeddings_static[torch.LongTensor([item_id])]
    
    feature_tensor = Variable(torch.zeros(1, len(feature_sequence[0])))
    user_timediffs_tensor = Variable(torch.zeros(1))
    item_timediffs_tensor = Variable(torch.zeros(1))
    item_embedding_previous = item_embeddings[torch.LongTensor([item_id])]
    ...
    user_item_embedding = torch.cat([user_projected_embedding, item_embedding_previous, 
                                     item_embeddings_static[torch.LongTensor([item_id])], user_embedding_static_input], dim=1)
